<template>
  <div style="margin-top: 10px" v-loading="fullloading">
    <div style="margin-bottom: 10px;display: flex;justify-content: center;align-items: center">
      <el-input
        placeholder="Show some users by default, you can search for more users by username..."
      ...
      <el-card style="width: 350px;margin-bottom: 20px" v-for="(item,index) in hrs" :key="item.id"
        ...
        <div>
          ...
          <div style="margin-top: 20px">
            <div><span class="user-info">Username:{{item.name}}</span></div>
            <div><span class="user-info">Phone Number:{{item.phone}}</span></div>
            <div><span class="user-info">Tel Number:{{item.telephone}}</span></div>
            <div><span class="user-info">Address:{{item.address}}</span></div>
            ...
            <div class="user-info">
              ...
              <el-popover
                ...
                :key="item.id"
                trigger="click">
                <el-select v-model="selRoles" multiple placeholder="Please select role">
                  <el-option
...
<!--                <i class="el-icon-more" style="color: #09c0f6;cursor: pointer" slot="reference"
            ...
            <div><span class="user-info">Remark:{{item.remark}}</span></div>
          </div>
        </div>
      </el-card>
    </div>
